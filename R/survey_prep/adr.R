...
adr.uid <- adr %>%
  ...
  summarise(
    # group = group[1],
    adr.disagree = 100 * mean(consensus == "disagree", na.rm = TRUE),
    ...
    adr.ind.bias = adr.ind.conf - adr.ind.acc,
    adr.grp.bias = adr.grp.conf - adr.grp.acc,
    adr.ind.discrimination = mean(Ind_Confidence.RESP[Ind_Stimulus.ACC == 1], na.rm = TRUE) - mean(Ind_Confidence.RESP[Ind_Stimulus.ACC == 0], na.rm = TRUE),
    adr.grp.discrimination = mean(Grp_Confidence.RESP[Grp_Stimulus.ACC == 1], na.rm = TRUE) - mean(Grp_Confidence.RESP[Grp_Stimulus.ACC == 0], na.rm = TRUE),
...
data <- adr %>%
  ungroup() %>% 
  select(uid, ADRitemNum, Ind_Confidence.RESP, Ind_Decision.RESP) %>%
  group_by(uid) %>% 
...
adr.uid <- adr.uid %>% left_join(post, by = "uid")

# prepare group data
data <- adr %>%
  ungroup() %>% 
  select(uid, ADRitemNum, Grp_Confidence.RESP, Grp_Decision.RESP) %>%
  group_by(uid) %>% 
