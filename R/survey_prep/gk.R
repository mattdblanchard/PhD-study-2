...
gk <- gk %>%
  mutate(Question = ifelse(Question == "Which state/territory has a larger area?", "Which state has a larger area?",
                           ifelse(Question == "Which state/territory has a larger length of coastline?", "Which state has a larger length of coastline?",
                                  ifelse(Question == "Which state/territory has a larger area?", "Which state has a larger area?",
                                         ifelse(Question == "Which state/territory has a larger population?", "Which state has a larger population?", Question)))))


check <- unique(gk %>% filter(version == c("a1", "b1")) %>% select(Itemnum, Question, Stimulus.CRESP))
check <- unique(gk %>% filter(version == "b1") %>% select(Itemnum, Question, Stimulus.CRESP))

# modify Itemnum
{
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "2"] <- "1"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "31"] <- "2"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "68"] <- "3"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "87"] <- "4"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "118"] <- "5"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "15"] <- "6"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "25"] <- "7"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "70"] <- "8"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "97"] <- "9"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "120"] <- "10"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "19"] <- "11"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "28"] <- "12"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "76"] <- "13"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "99"] <- "14"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "123"] <- "15"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "21"] <- "16"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "23"] <- "17"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "79"] <- "18"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "103"] <- "19"
gk$Itemnum[gk$version %in% c("a1", "b1") & gk$Itemnum == "139"] <- "20"
}

# gk that itemnums were correctly modified
item <- unique(gk %>% mutate(Itemnum = as.numeric(Itemnum)) %>% select(Itemnum, Stimulus.CRESP)) %>% arrange(Itemnum)
print(item)
item <- item$Itemnum
...
# discovered that uid == 18042010_2_d1 accidentally entered the wrong response for item 18 - should be 1
gk <- gk %>% 
  mutate(Grp_Stimulus.RESP = ifelse(uid == "18042010_2_d1" & Itemnum == 18, 1, Grp_Stimulus.RESP))

# add Stimulus.ACC column (Stimulus.RESP == Stimulus.CRESP)
# first need to modift Stimulus.CRESP as 3 items have the wrong CRESP recorded
gk <- gk %>% 
  mutate(Stimulus.CRESP = ifelse(Itemnum == "14", 1, 
                                 ifelse(Itemnum == "17", 1, 
                                        ifelse(Itemnum == "18", 1, Stimulus.CRESP))),
         Stimulus.CRESP = as.numeric(Stimulus.CRESP)) %>% 
  group_by(uid, Itemnum) %>%
...
# overall GK variables
gk.uid <- gk %>%
  group_by(uid) %>%
  summarise(
    # group = group[1],
    gk.disagree = 100 * mean(consensus == "disagree", na.rm = TRUE),
    gk.ind.accO = mean(Ind_Stimulus.ACC, na.rm = TRUE) * 100,
    gk.grp.accO = mean(Grp_Stimulus.ACC, na.rm = TRUE) * 100,
    gk.ind.confO = mean(Ind_Confidence.RESP, na.rm = TRUE),
    gk.grp.confO = mean(Grp_Confidence.RESP, na.rm = TRUE),
    gk.ind.biasO = gk.ind.confO - gk.ind.accO,
    gk.grp.biasO = gk.grp.confO - gk.grp.accO,
    gk.ind.discriminationO = mean(Ind_Confidence.RESP[Ind_Stimulus.ACC == 1], na.rm = TRUE) - mean(Ind_Confidence.RESP[Ind_Stimulus.ACC == 0], na.rm = TRUE),
    gk.grp.discriminationO = mean(Grp_Confidence.RESP[Grp_Stimulus.ACC == 1], na.rm = TRUE) - mean(Grp_Confidence.RESP[Grp_Stimulus.ACC == 0], na.rm = TRUE),
    gk.ind.decO = mean(Ind_Decision.RESP == "y", na.rm = TRUE) * 100,
...
data <- gk %>%
  ungroup() %>% 
  select(uid, Itemnum, Ind_Confidence.RESP, Ind_Decision.RESP) %>%
  group_by(uid) %>% 
...
gk.uid <- gk.uid %>% left_join(post, by = "uid")

# prepare group data
data <- gk %>%
  ungroup() %>% 
  select(uid, Itemnum, Grp_Confidence.RESP, Grp_Decision.RESP) %>%
  group_by(uid) %>% 
...
gk.rep <- gk %>%
  group_by(uid, Representativeness) %>%
  summarise(
    # group = group[1],
    gk.ind.acc = mean(Ind_Stimulus.ACC, na.rm = TRUE) * 100,
    gk.grp.acc = mean(Grp_Stimulus.ACC, na.rm = TRUE) * 100,
    gk.ind.conf = mean(Ind_Confidence.RESP, na.rm = TRUE),
    gk.grp.conf = mean(Grp_Confidence.RESP, na.rm = TRUE),
    gk.ind.bias = gk.ind.conf - gk.ind.acc,
    gk.grp.bias = gk.grp.conf - gk.grp.acc,
    gk.ind.discrimination = mean(Ind_Confidence.RESP[Ind_Stimulus.ACC == 1], na.rm = TRUE) - mean(Ind_Confidence.RESP[Ind_Stimulus.ACC == 0], na.rm = TRUE),
    gk.grp.discrimination = mean(Grp_Confidence.RESP[Grp_Stimulus.ACC == 1], na.rm = TRUE) - mean(Grp_Confidence.RESP[Grp_Stimulus.ACC == 0], na.rm = TRUE),
    gk.ind.dec = mean(Ind_Decision.RESP == "y", na.rm = TRUE) * 100,
  ...
  gather(var, val, -uid, -Representativeness) %>% 
...
data <- gk %>%
  ungroup() %>% 
  select(uid, Representativeness, Itemnum, Ind_Confidence.RESP, Ind_Decision.RESP) %>%
  group_by(uid, Representativeness) %>% 
  filter(!all(is.na(Ind_Decision.RESP))) %>% 
  mutate(
    Ind_Decision.RESP = as.numeric(ifelse(Ind_Decision.RESP == "y", 1,
...
data <- gk %>%
  ungroup() %>% 
  select(uid, Representativeness, Itemnum, Grp_Confidence.RESP, Grp_Decision.RESP) %>%
  group_by(uid, Representativeness) %>% 
  filter(!all(is.na(Grp_Decision.RESP))) %>% 
  mutate(
    Grp_Decision.RESP = as.numeric(ifelse(Grp_Decision.RESP == "y", 1,
