...
data <- crt %>%
  ungroup() %>% 
  select(uid, consensus, CRTItemNum, Ind_Confidence.RESP, Ind_Decision.RESP) %>%
  group_by(uid, consensus) %>% 
...
crt.uid <- crt.uid %>% left_join(post, by = c("uid", "consensus"))

# prepare group data
data <- crt %>%
  ungroup() %>% 
  select(uid, consensus, CRTItemNum, Grp_Confidence.RESP, Grp_Decision.RESP) %>%
  group_by(uid, consensus) %>% 
